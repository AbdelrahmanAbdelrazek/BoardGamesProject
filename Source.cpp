...
list<Player>* getPlayers() {
	...
	GETSYMBOL:
		...
		if (input.length() == 1) {
			...
			cout << "please enter one symbol" << endl;
			goto GETSYMBOL;
		}
		...
		//See if the user wishes to be more than 2 players
		if (players->size() >= 2) {
			cout << "Add another player? (Y/N)" << endl;
		ADDANOTHERPLAYER:
			getline(cin, input);
			if (input == "n" || input == "N") {
				break;
				...
				cout << "Please enter only Y for yes or N for No " << endl;
				goto ADDANOTHERPLAYER;
			}
		}
		cout << endl;
	}
...
void main() {
	...
	cout << "Hello And Welcome To Board games v1.032" << endl << endl << endl;
	...
	TRYAGAIN:
		...
		cout << "Enter 2 to play connect4 " << endl;
		gameNo = getInt();
		if (gameNo == 0)
			break;
